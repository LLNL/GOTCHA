...
static int mark_got_writable(struct link_map *lib)
{
   ...
   if(protect_size % page_size){
      protect_size += page_size -  ((protect_size) %page_size); // GCOVR_EXCL_LINE
   }
   ...
   if(res == -1){ // mprotect returns -1 on an error
      error_printf("GOTCHA attempted to mark the GOT table as writable and was unable to do so, " // GCOVR_EXCL_LINE
