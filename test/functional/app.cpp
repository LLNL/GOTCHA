...
int main()
{
   ...
   gotcha_quick_wrap(malloc,[&](original_malloc orig, unsigned long size){
   ...
   gotcha_quick_wrap(free,[&](original_free orig, free_arg_type to_free){
       last_freed_address = to_free;
       std::cout << "Freeing " << (void*)to_free << std::endl;
       orig(to_free);
   });

   int* test_allocation = (int*) malloc(sizeof(int) * 1000);
